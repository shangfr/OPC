  ...
  try {
    ...
    const stream = createUIMessageStream({
      ...
      execute: async ({ writer: dataStream }) => {
        ...
        const result = await withRetry(
          () => streamText({
            ...
            providerOptions: (() => {
              ...
              //
              // 根据 DeepSeek 官方文档（https://api-docs.deepseek.com/zh-cn/guides/thinking_mode）：
              // 1. thinking 参数控制思考开关：{type:"enabled"} 开启 / {type:"disabled"} 关闭
              //    - 默认 enabled
              //    - 必须通过 extra_body 传递（OpenAI SDK），对应 @ai-sdk/openai-compatible
              //      的 providerOptions[providerName] 透传机制
              // 2. reasoning_effort 控制思考强度：只接受 "high" / "max"
              //    - low/medium 会映射为 high，xhigh 会映射为 max
              //    - 对普通请求默认 high，复杂 Agent 请求自动 max
              //
              // @ai-sdk/openai-compatible 参数传递机制：
              // - schema 字段（reasoningEffort）：从 openaiCompatible / {providerName} 解析，
              //   映射为请求体的 reasoning_effort 字段
              // - 非 schema 字段（thinking）：只从 providerOptions[{providerName}] 透传到请求体
              //
              // 因此：
              // - reasoningEffort 放在 openaiCompatible 下（开启时传 high/max）
              // - thinking 放在 {providerName} 下（如 deepseek / zhipu）
              if (!isReasoningModel) return undefined;

              const providerName = modelConfig?.provider;
              const opts: Record<string, unknown> = {};

              // reasoning_effort：仅在开启思考时传递（high/max）
              // 关闭思考时不传 reasoning_effort（让模型用默认值，由 thinking:disabled 控制）
              if (thinkingEnabled) {
                opts.openaiCompatible = {
                  reasoningEffort: modelConfig?.reasoningEffort ?? "high",
                };
              }

              // thinking 参数：控制思考开关（DeepSeek 和智谱 GLM 都支持）
              // provider name 见 config.ts：deepseek → "deepseek"，zhipuai → "zhipu"
              const thinkingKey =
                providerName === "zhipuai" ? "zhipu" : providerName;
              if (thinkingKey) {
                opts[thinkingKey] = {
        ...
        // 根据 DeepSeek 官方文档：
        // - thinking: {type:"disabled"} 时，模型不输出 reasoning_content
        // - thinking: {type:"enabled"} 时，模型先输出 reasoning_content，再输出 content
        //
        // sendReasoning 控制是否将 reasoning_content 转发到前端：
        // - 始终 sendReasoning: true，确保 reasoning 和 text 事件都能正常流式输出
        //   （sendReasoning: false 会过滤 reasoning-delta 但仍发送 reasoning-start/end，
        //    导致前端创建空 reasoning part，可能影响 text part 的渲染时机）
        // - 关闭思考时，前端 message.tsx 中 isLoading && !thinkingEnabled 不渲染 reasoning
        // - 关闭思考时，onFinish 中过滤 reasoning parts，不写入数据库
        dataStream.merge(
      ...
      onFinish: async ({ messages: finishedMessages }) => {
        // 关闭思考模式时，过滤掉 reasoning parts，不写入数据库。
